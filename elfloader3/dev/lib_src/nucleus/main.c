...
void ElfKiller(void)
{
    /* разблокируем мютекс тем самым разрешив довыполнитс€ коду в _main и убить эльф */
    MutexUnlock(&mutex);
    kill_me = 1;
}
...
int _main(char *exename, char *fname)
{
    ...
    /* ждем пока мютекс не разблокируетс€ */
    MutexLock(&mutex);

    /* если мютекс разблокировали - удал€ем его */
    MutexDestroy(&mutex);
    ...
    /* после завершени€ функции процесс убьЄтс€ автоматически */
    return 0;
}


/* вотето должно происходить в эльфпаке */
int main()
{
    /* создаЄм процесс с именем "Test" в группе 0x43 и приоритетом 0x80 */
